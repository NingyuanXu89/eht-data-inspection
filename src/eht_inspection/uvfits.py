...
# def plot_scan_phase_vs_time_all_baselines(
#     allcoh,
#     t_unique,
#     station_list,
#     channel_list=None,
...
# ):
#     """
#     Plot phase vs time for all baselines in one scan, using one selected channel.
...
#     t_unique : ndarray
#         Unique time values for this scan.

#     station_list : ndarray
#         Station names for this scan.

#     channel_list : ndarray or None
#         Channel labels/indices. If provided, `channel` is interpreted as
#         the actual channel label in channel_list. If None, `channel` is
#         interpreted directly as a channel index.

#     channel : int
#         Channel to use for the time series.

#         If channel_list is None:
#             channel is used as an array index.

#         If channel_list is provided:
#             channel is matched against channel_list, and the corresponding
#             array index is used.

#     scan_num : int or None
#         Scan number used in the figure suptitle and filename.

#     unwrap_phase : bool
#         If True, unwrap phase along the time axis.
...
#     source : str
#         Source name used in the figure filename/suptitle.
...
#             raise ValueError(
#                 f"channel={channel} not found in channel_list={channel_list}"
#             )
...
#         raise IndexError(
#             f"Selected channel index {ichan} is out of range for Nc={allcoh.shape[1]}"
#         )
...
#             # V_time has shape Nt
#             V_time = allcoh[:, ichan, i, j, p, q]

#             # Treat zero-filled missing entries as invalid.
#             V_time = np.where(V_time == 0, np.nan + 1j * np.nan, V_time)

#             if unwrap_phase:
#                 phase = np.unwrap(np.angle(V_time)) * 180.0 / np.pi
#             else:
#                 phase = np.angle(V_time, deg=True)
...
#     if scan_num is None:
#         suptitle = (
#             f"{source}: phase vs time for all baselines, "
#             f"channel {channel_label}{unwrap_text}"
#         )
#     else:
#         suptitle = (
#             f"{source}, scan {scan_num}: phase vs time for all baselines, "
#             f"channel {channel_label}{unwrap_text}"
#         )
...
#         if scan_num is None:
#             filename = f"{source}_phase_vs_time_ch{channel_label}_all_baselines.png"
#         else:
#             filename = f"{source}_scan{scan_num}_phase_vs_time_ch{channel_label}_all_baselines.png"
...
# def plot_scan_amp_vs_time_all_baselines(
#     allcoh,
#     t_unique,
#     station_list,
#     channel_list=None,
...
# ):
#     """
#     Plot amplitude vs time for all baselines in one scan, using one selected channel.
...
#     if scan_num is None:
#         suptitle = (
#             f"{source}: amplitude vs time for all baselines, "
#             f"channel {channel_label}"
#         )
#     else:
#         suptitle = (
#             f"{source}, scan {scan_num}: amplitude vs time for all baselines, "
#             f"channel {channel_label}"
#         )
...
#         if scan_num is None:
#             filename = f"{source}_amp_vs_time_ch{channel_label}_all_baselines.png"
#         else:
#             filename = f"{source}_scan{scan_num}_amp_vs_time_ch{channel_label}_all_baselines.png"
